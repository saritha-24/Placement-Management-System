...
<body> 
	<table>
		...
		$dis="SELECT * FROM `odform` WHERE `tutor`='approved' AND `hod`='none'";
...
<?php 
	...
	$sno=$_POST['sno'];
	$con=mysqli_connect("localhost:3306","root","","placement");
	$dis="UPDATE `odform` SET `hod`='approved' WHERE `sno`='$sno'";
	$r=mysqli_query($con,$dis);
	if($r)
	{
	...
	$sno=$_POST['sno'];
	$con=mysqli_connect("localhost:3306","root","","placement");
	$dis="UPDATE `odform` SET `hod`='declined' WHERE `sno`='$sno'";
	$r=mysqli_query($con,$dis);
	if($r)
	{
